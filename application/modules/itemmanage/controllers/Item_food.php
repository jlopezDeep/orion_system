...
class Item_food extends MX_Controller
{
		...
		if ($this->form_validation->run()) {
			...
			if (empty($this->input->post('ProductsID'))) {
				...
				$data['foodlist']   = (object) $postData = array(
					...
					'color'   	            => $this->input->post('color', true), //Color
					'bodegon'   	        => $this->input->post('bodegon', true),
				);
				$logData = array(
				...
				if ($this->fooditem_model->fooditem_create($postData)) {
					...
					//==========================
					if ($this->input->post('create_variant', true) == '1') {
						$price = $this->input->post('price', true);
						//----------------------------
						if ($this->input->post('price_tax', true) == '1') {
							$divisor = (1 + ($myvat / 100));
							$price = $price / $divisor;
						}
						//Variante
						$varianteData = array(
							'variantName' => $this->input->post('foodname', true),
							'menuid' => $itemfoodid,
							'price' => $price,
						);
						$this->db->insert('variant', $varianteData);
						$varientid = $this->db->insert_id();
					}

					if ($this->input->post('bodegon', true) == '1') {
						//Ingrediente
						...
						$this->db->insert('ingredients', $ingredientData);
						$ingredientid = $this->db->insert_id();
						//Produccion unitaria
						$productionData = array(
							...
							'qty' => 1,
							'createdby' => $savedid,
							'pvarientid' => $varientid
						);
						$this->db->insert('production_details', $productionData);
					}
					//==========================
		...
		if (!empty($_FILES["userfile"]["name"])) {
			...
			foreach ($object->getWorksheetIterator() as $sale) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					$category = $sale->getCellByColumnAndRow(0, $row)->getValue();
					$parendcategory = $sale->getCellByColumnAndRow(1, $row)->getValue();
					$kitchen = $sale->getCellByColumnAndRow(2, $row)->getValue();
					$itemname = $sale->getCellByColumnAndRow(3, $row)->getValue();
					$description = $sale->getCellByColumnAndRow(4, $row)->getValue();
					$status = $sale->getCellByColumnAndRow(5, $row)->getValue();
					$varient = $sale->getCellByColumnAndRow(6, $row)->getValue();
					$price = $sale->getCellByColumnAndRow(7, $row)->getValue();
	...
	public function downloadformat()
	{
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('B1', 'Parent Category');
		$objPHPExcel->getActiveSheet()->SetCellValue('C1', 'Kitchen Name');
		$objPHPExcel->getActiveSheet()->SetCellValue('D1', 'Food Name');
		$objPHPExcel->getActiveSheet()->SetCellValue('E1', 'Description');
		$objPHPExcel->getActiveSheet()->SetCellValue('F1', 'Status');
		$objPHPExcel->getActiveSheet()->SetCellValue('G1', 'Varient Name');
		...
		foreach ($arrayfood as $row) {
			$objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, $row['category'], 'UTF-8');
			$objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $row['parent'], 'UTF-8');
			$objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $row['kitchen'], 'UTF-8');
			$objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $row['item'], 'UTF-8');
			$objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $row['description'], 'UTF-8');
			$objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $row['status'], 'UTF-8');
			$objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $row['varient'], 'UTF-8');
			$objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $row['price'], 'UTF-8');
			$rowCount++;
		}
	...
	public function varientcreate($id = null)
	{
		...
		$infotax = $this->db->select('*')->from('item_foods')->where('ProductsID', $this->input->post('foodid', true))->get()->result_array();
		$divisor = (1 + ($infotax[0]["productvat"] / 100));
		if ($this->input->post('price_type')) {
			$data['varient']   = (object) $postData = [
				...
				'price' 	 	    => $this->input->post('price', true) / $divisor,
			];
		} else {
			$data['varient']   = (object) $postData = [
				'variantid'          => $this->input->post('variantid'),
