...
class Production_model extends CI_Model
{
	...
	{
		//join('unit_of_measurement', 'ingredients.uom_id=unit_of_measurement.id', 'left')
		$data = $this->db->select("*")->from('ingredients')->where('ingredients.is_active', 1)->get()->result();
	...
	public function checkingredientstock($foodid, $vid, $foodqty)
	{
			...
			foreach ($groupitemlist as $groupitem) {
				...
				if (empty($productiondetails)) {
					...
					foreach ($productiondetails as $productiondetail) {
						...
						if ($stockcheck == 0) {
							return 'No tiene stok en sus ingredientes!!!' . $groupitem->items;
						}
						/*end add ingredients*/
					}
				}
		...
		if (!empty($productiondetails)) {
			...
			foreach ($productiondetails as $productiondetail) {
				...
				if ($stockcheck == 0) {
					return 'No tiene stock en sus ingredientes!!!';
				}
			...
			return 'No se ha definido una receta para este producto';
		}
		return 1;
	}
