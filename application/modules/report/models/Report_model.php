...
class Report_model extends CI_Model
{
	...
	public function salereport($start_date, $end_date, $pid = null, $invoice_id = null, $journal = null, $is_gif)
	{
		//echo $journal;
		if ($journal == null) {
			$dateRange = "a.order_date BETWEEN '$start_date%' AND '$end_date%' AND a.order_status=4";
			if ($pid != null && $invoice_id == null) {
				$dateRange = "a.order_date BETWEEN '$start_date%' AND '$end_date%' AND a.order_status=4 AND mp.payment_type_id=$pid";
			}
			if ($invoice_id != null) {
				$dateRange = "a.order_status=4 AND a.saleinvoice=$invoice_id";
			}
			if ($is_gif == "1") {
				$dateRange =  $dateRange . " AND a.totalamount = 0";
				//echo $dateRange;
			} else {
				$dateRange =  $dateRange . " AND a.totalamount != 0";
				//echo $dateRange;
			...
			$dateRange = "a.journal_id=" . $journal . " AND a.order_status=4";
			if ($pid != null && $invoice_id == null) {
				$dateRange = "a.journal_id=" . $journal . " AND a.order_status=4 AND mp.payment_type_id=" . $pid;
			}
			if ($invoice_id != null) {
				$dateRange = "a.journal_id=" . $journal . " AND a.order_status=4 AND a.saleinvoice=" . $invoice_id;
			}
			if ($is_gif == "1") {
				$dateRange =  $dateRange . " AND a.totalamount = 0";
				//echo $dateRange;
		...
		$this->db->select("t.*, a.*,b.customer_id,b.customer_name,b.customer_id,c.*,p.*, mp.*");
		...
		$this->db->join('rest_table t', 'a.table_no=t.tableid', 'left');
		$this->db->join('multipay_bill mp', 'c.order_id=mp.order_id', 'left');
		$this->db->join('payment_method p', 'mp.payment_type_id=p.payment_method_id', 'left');
		$this->db->where($dateRange, NULL, FALSE);
	...
	public function journal_dropdown()
	{
		$data = $this->db->select("*")
			->from('journal')
			//->where('', 1)
	...
	public function itemsReport($start_date, $end_date, $journal = null)
	{
