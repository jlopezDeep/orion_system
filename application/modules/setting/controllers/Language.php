...
class Language extends MX_Controller {
    ...
    public function phrase()
    {
 	...
 	public function downloadformat(){
				...
				$i++;
                    if ($i > 2){
					$result[$i] = $field->name;
					$objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$i]."1", $field->name);
					}
                }
				$objPHPExcel->getActiveSheet()->getStyle($alphabet[3].":".$alphabet[$lastnum])->getFont()->setBold(true);
				//print_r($objPHPExcel);
				$rowCount   =   2;
				...
				foreach($languagelist as $row){
					...
					foreach($result as $flname){
						$k++;
							$objPHPExcel->getActiveSheet()->SetCellValue($alphabet[$k].$rowCount, $row->$flname,'UTF-8');
						}
						$rowCount++;
					}
	...
	public function bulklanupload(){
		$this->permission->method('setting','read')->redirect();
			if(!empty($_FILES["userfile"]["name"])) {
                ...
                {
                $phase=$sale->getCellByColumnAndRow(0, $row)->getValue();
						...
						$i++;
							...
							echo $i;
								(Object) $languageData = array(
									...
									$field->name     	=> $sale->getCellByColumnAndRow($k, $row)->getValue()
									);
	...
	public function downloadformatsingle($language){
		...
		if($language=="english"){
			$objPHPExcel->getActiveSheet()->SetCellValue("A1", "phrase");
			$objPHPExcel->getActiveSheet()->SetCellValue("B1", $language);
			$objPHPExcel->getActiveSheet()->getStyle("A1:B1")->getFont()->setBold(true);
			$languagelist=$this->db->select('phrase,'.$language)->from($this->table)->order_by('phrase','asc')->get()->result();
			foreach($languagelist as $row){
				$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $row->phrase,'UTF-8');
				$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row->$language,'UTF-8');
				$rowCount++;
			}
		}else{
				$objPHPExcel->getActiveSheet()->SetCellValue("A1", "phrase");
				$objPHPExcel->getActiveSheet()->SetCellValue("B1", "english");
				$objPHPExcel->getActiveSheet()->SetCellValue("C1", $language);
				$objPHPExcel->getActiveSheet()->getStyle("A1:C1")->getFont()->setBold(true);
				$languagelist=$this->db->select('phrase,english,'.$language)->from($this->table)->order_by('phrase','asc')->get()->result();
				foreach($languagelist as $row){
					$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $row->phrase,'UTF-8');
					$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row->english,'UTF-8');
					$objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, $row->$language,'UTF-8');
					$rowCount++;
				}
	...
	public function bulklanuploadsingle($language){
		$this->permission->method('setting','read')->redirect();
			if(!empty($_FILES["userfile"]["name"])) {
                ...
                $phase=$sale->getCellByColumnAndRow(0, $row)->getValue();
				if($language=="english"){
					$targetlan=$sale->getCellByColumnAndRow(1, $row)->getValue();
				}else{
					$english=$sale->getCellByColumnAndRow(1, $row)->getValue();
					$targetlan=$sale->getCellByColumnAndRow(2, $row)->getValue();
					}
						(Object) $languageData = array(
									$language	=> $targetlan
									);
