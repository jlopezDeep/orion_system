<?php
require 'print_server/escpos-php/autoload.php';
...
class Order extends MX_Controller
{
	...
	public function insert_customer()
	{
		...
		if ($this->form_validation->run()) {
			...
			$data['customer']   = (object) $postData = array(
				'cuntomer_no'     	=>  $this->input->post('customer_no', true), //$sino,
	...
	public function pos_invoice()
	{
		...
		$passengerinfo = "<p>Ticket Purpose Details:</p><p>Secondary Passenger: </p><p>" . $emplist . "</p>";
		...
		//============================================
		$data["parameters"] = $this->paramSystem();
		$data["currency_rate"] = $this->db->select('*')->from('currency')->where('is_secondary_currency', 1)->get()->row();
		$data["station_info"] = $this->db->select('*')->from('station')->where('ip', $this->getRealIP())->get()->row();
		//============================================
		$data['module'] = "ordermanage";
		$data['page']   = "posorder";
		echo Modules::run('template/layout', $data);
	...
	public function cartupdateQty($cartID, $qty)
	{
		//$this->permission->method('ordermanage', 'read')->redirect();
		//$cartID = $this->input->post('CartID');
	...
	{
		//DESBLOQUEAMOS LA MESA
		$updatetDataTable = array('user_view' => NULL);
		$this->db->where('user_view', $this->session->userdata('id'));
		$this->db->update('customer_order', $updatetDataTable);
		$this->cart->destroy();
		//redirect('ordermanage/order/pos_invoice');
	}
		...
		if ($this->form_validation->run()) {
			if ($cart = $this->cart->contents()) {
				...
				$data2 = array(
					...
					'waiter_id'	        	=>	$this->session->userdata('id'), //$this->input->post('waiter', true),
					...
					'order_status'		    =>	1,
					'journal_id'            =>  $_SESSION['journal'],
					'ip_origin'             =>  $ip_origin,
					'is_update'             =>  1,
					'comentario'            =>  $comentario
				);

				$this->db->query("update station set is_update=NULL");
				...
				if ($this->order_model->orderitem($orderid)) {
					...
					//=====================================================================
					$kitchenlistcontrol = $this->db->select('kitchenid as kitchen_id,kitchen_name,ip,port')->from('tbl_kitchen')->where('is_print_control', 1)->order_by('kitchen_name', 'Asc')->get()->result();
					//=====================================================================================================================================================
					...
					//=====================================================================================================================================================
					//===================
					$infoparams = $this->db->select('*')->from('system_param')->get()->result_array();
					//====================================================================
					if ($infoparams[5]["active"] == '0') { //comanda activa
						foreach ($alliteminfo as $kitchen) {
							$print_id = 0;
							if ($kitchen->kitchenid != $print_id) {
								//--------------------------------
								$print_id = $kitchen->kitchenid;
								//--------------------------------------------------------------------
								try {
									$path = $infoparams[1]["value"] . "P_" . $kitchen->ip . "-" . $DateAndTimeId . ".printer";
									...
									if ($archivo == false) {
										...
										if ($singleorderinfo->tablename != "") {
											$table = $singleorderinfo->tablename;
										} else {
											$table = "ORDEN LIBRE";
										}
										// Escribir en el archivo:
										fwrite($archivo, "ORDEN: " . $singleorderinfo->order_id . "\r\n");
										fwrite($archivo, "MESA: " . $table . "\r\n");
										fwrite($archivo, "CLIENTE: " . $singleorderinfo->customer_name . "\r\n");
										fwrite($archivo, "ATENDIO: " . $singleorderinfo->first_name . "\r\n");
										fwrite($archivo, "AREA: " . $kitchen->kitchen_name . "\r\n");
										fwrite($archivo, "COMENTARIO: " . $singleorderinfo->comentario . "\r\n");
										fwrite($archivo, "------------------------------------------------\r\n");
										fwrite($archivo, "PRODUCTOS \r\n");
										fwrite($archivo, "------------------------------------------------\r\n");
										foreach ($alliteminfo as $item) {
											if ($item->kitchenid == $kitchen->kitchenid) {
												fwrite($archivo, $item->menuqty . "x " . $item->variantName . "\r\n");
												//$printer-> text("----- ".$item->variantName."\n");
												if ($item->add_on_name != '') {
													fwrite($archivo, "   (A) " . $item->addonsqty . "x " . $item->add_on_name . "\r\n");
												}
												if ($item->notes != '') {
													fwrite($archivo, "   (M) " . $item->notes . "\r\n");
												}
											}
										}
										fwrite($archivo, "------------------------------------------------\r\n");
										fwrite($archivo, "FECHA: " . $DateAndTime . "\r\n");
										fwrite($archivo, "IMPRESORA: " . $kitchen->ip . "\r\n");
						...
						//Comanda de control
						foreach ($kitchenlistcontrol as $kitchen) {
							try {
								$path = $infoparams[1]["value"] . "P_" . $kitchen->ip . "-" . $DateAndTimeId . "-CONTROL.printer";
								$archivo = fopen($path, "wb");
								if ($archivo == false) {
									echo "Error al crear el archivo";
								} else {
									...
									// Escribir en el archivo:
									fwrite($archivo, "------------------------------------------------\r\n");
									fwrite($archivo, "COMANDA DE CONTROL \r\n");
									fwrite($archivo, "------------------------------------------------\r\n");
									fwrite($archivo, "ORDEN: " . $singleorderinfo->order_id . "\r\n");
									fwrite($archivo, "MESA: " . $table . "\r\n");
									fwrite($archivo, "CLIENTE: " . $singleorderinfo->customer_name . "\r\n");
									fwrite($archivo, "ATENDIO: " . $singleorderinfo->first_name . "\r\n");
									fwrite($archivo, "AREA: TODAS\r\n");
									fwrite($archivo, "------------------------------------------------\r\n");
									fwrite($archivo, "PRODUCTOS \r\n");
									fwrite($archivo, "------------------------------------------------\r\n");
									$kitchenid = 0;
									foreach ($alliteminfo as $item) {
										...
										if ($kitchenid != $item->kitchenid) {
											$kitchenid = $item->kitchenid;
											fwrite($archivo, $item->kitchen_name . "\r\n");
										}
										fwrite($archivo, "   " . $item->menuqty . "x " . $item->variantName . "\r\n");
										//$printer-> text("----- ".$item->variantName."\n");
										if ($item->add_on_name != '') {
		...
		foreach ($list as $rowdata) {
			...
			if ($this->permission->method('ordermanage', 'read')->access()) :
				if (($rowdata->order_status != 5 && $rowdata->orderacceptreject != 1) && ($rowdata->cutomertype == 2 || $rowdata->cutomertype == 99)) {
					$acptreject = '&nbsp;<a href="javascript:;" id="accepticon_' . $rowdata->order_id . '" data-id="' . $rowdata->order_id . '" class="btn btn-xs btn-danger btn-sm mr-1 aceptorcancel" data-toggle="tooltip" data-placement="left" title="" data-original-title="Accept or Cancel"><i class="fa fa-info-circle"></i></a>&nbsp;';
				}
				if ($rowdata->order_status == 1 || $rowdata->order_status == 2 || $rowdata->order_status == 3) {
					$cancelbtn = '&nbsp;<a href="javascript:;" id="cancelicon_' . $rowdata->order_id . '" data-id="' . $rowdata->order_id . '" class="btn btn-xs btn-danger btn-sm mr-1 aceptorcancel" data-toggle="tooltip" data-placement="left" title="" data-original-title="Accept or Cancel"><i class="fa fa-trash-o"></i></a>&nbsp;';
	...
	public function todayallorder()
	{
		...
		foreach ($list as $rowdata) {
			...
			$row[] = $rowdata->firstname . $rowdata->lastname;
			$row[] = $rowdata->tablename;
			$row[] = $rowdata->order_date;
			$row[] = $rowdata->totalamount;
			$row[] = $rowdata->order_time;
	...
	public function acceptnotify()
	{
		...
		foreach ($iteminfo as $kitchen) {
			...
			if ($kitchen->kitchenid != $print_id) {
				...
				//--------------------------------------------------------------------
				try {
					$path = $infoparams[1]["value"] . "P_" . $kitchen->ip . "-" . $DateAndTimeId . "-ANULACION_COMPLETA.printer";
					$archivo = fopen($path, "wb");
					if ($archivo == false) {
						echo "Error al crear el archivo";
					} else {
						// Escribir en el archivo:
						fwrite($archivo, "************************************************\r\n");
						fwrite($archivo, "ORDEN ELIMINADA: \r\n");
						fwrite($archivo, "************************************************\r\n");
						fwrite($archivo, "ANULACION COMPLETA: \r\n");
						fwrite($archivo, "************************************************\r\n");
						fwrite($archivo, "------------------------------------------------\r\n");
						fwrite($archivo, "ORDEN: " . $singleorderinfo->order_id . "\r\n");
						fwrite($archivo, "MESA: " . $singleorderinfo->tablename . "\r\n");
						fwrite($archivo, "CLIENTE: " . $singleorderinfo->customer_name . "\r\n");
						fwrite($archivo, "ATENDIO: " . $singleorderinfo->first_name . "\r\n");
						fwrite($archivo, "AREA: " . $kitchen->kitchen_name . "\r\n");
						fwrite($archivo, "------------------------------------------------\r\n");
		...
		if ($acceptreject == 1) {
			...
			$this->db->where('id', $orderid)->delete('table_details');
		}
		if ($acceptreject == 1) {
		...
		curl_close($ch);*/
		$this->delete_table_details($orderid);
		$this->db->query("update station set is_update=NULL");
		if ($onprocesstab == 1) {
			$data['ongoingorder']  = $this->order_model->get_ongoingorder();
			$data['module'] = "ordermanage";
	...
	public function printtoken()
	{
		...
		$DateAndTimeId = date('d-m-Y-h-i-s-a', time());
		//$uuid = gen_uuid();
		$kitchenlist = $this->db->select('kitchenid as kitchen_id,kitchen_name,ip,port')->from('tbl_kitchen')->where('kitchenid', $kid)->order_by('kitchen_name', 'Asc')->get()->result();
		foreach ($kitchenlist as $kitchen) {
			...
			try {
				...
				if ($archivo == false) {
					...
					// Escribir en el archivo:
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "REENVIO DE COMANDA: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "ORDEN: " . $singleorderinfo->order_id . "\r\n");
					fwrite($archivo, "MESA: " . $table . "\r\n");
					fwrite($archivo, "CLIENTE: " . $singleorderinfo->customer_name . "\r\n");
	...
	public function updateorder($id)
	{
		...
		$data['orderinfo'] = $this->order_model->kitchen_ajaxorderinfoall($id);
		...
		$data['taxinfos'] = $this->taxchecking();
		$data["currency_rate"] = $this->db->select('*')->from('currency')->where('is_secondary_currency', 1)->get()->row();
		//$data['currency_tasa'] = $this->db->select('*')->from('currency')->where('is_secondary_currency', 1)->get()->result_array();
		$data["station_info"] = $this->db->select('*')->from('station')->where('ip', $this->getRealIP())->get()->row();
		//BLOQUEAMOS LA MESA
		/*$updatetDataTable = array('user_view' => 1);
		$this->db->where('tableid', $customerorder->table_no);
		$this->db->update('rest_table', $updatetDataTable);*/
	...
	public function modifyoreder()
	{
		...
		$iteminfo = $this->order_model->customerorder($orderid, 1); //el uno es para solo los actualizados
		$infoparams = $this->db->select('*')->from('system_param')->get()->result_array();
		//Imprimir comanda de cocina
		//****************************************************/
		date_default_timezone_set("America/Caracas");
		...
		$this->db->where('order_id', $orderid);
		$this->db->update('order_menu', $orderUpdateData);
		//$this->db->query("update customer_order set is_update=1 where order_id = " . $orderid);
		//==============================================================
		...
		//=================================
		//BLOQUEAMOS LA MESA
		$updatetDataTable = array('user_view' => $this->session->userdata('id'));
		$this->db->where('order_id', $orderid);
		$this->db->update('customer_order', $updatetDataTable);
		//====================================
		$this->db->query("update customer_order set is_update=1 where order_id = " . $orderid);
	...
	public function deletetocart()
	{
		...
		if ($isupdate == 0) {
			foreach ($iteminfo as $kitchen) {
				if ($kitchen->row_id == $rowid && $kitchen->isupdate != '1') {
					$print_id = 0;
					if ($kitchen->kitchenid != $print_id) {
						//--------------------------------
						$print_id = $kitchen->kitchenid;
						//--------------------------------------------------------------------
						try {
							$path = $infoparams[1]["value"] . "P_" . $kitchen->ip . "-" . $DateAndTimeId . "-ANULACION.printer";
							$archivo = fopen($path, "wb");
							if ($archivo == false) {
								echo "Error al crear el archivo";
							} else {
	...
	public function dueinvoice($id)
	{
		...
		$infoparams = $this->db->select('*')->from('system_param')->get()->result_array();
		$currency_tasa = $this->db->select('*')->from('currency')->where('is_secondary_currency', 1)->get()->result_array();
		//Imprimir comanda de cocina
		//****************************************************/
		date_default_timezone_set("America/Caracas");
		$DateAndTime = date('d-m-Y h:i:s a', time());
		...
		foreach ($kitchenlist as $kitchen) {
			//--------------------------------------------------------------------
			try {
				$path = $infoparams[1]["value"] . "P_" . $kitchen->ip . "-" . $DateAndTimeId . "-PRECUENTA.printer";
				$archivo = fopen($path, "wb");
				if ($archivo == false) {
					echo "Error al crear el archivo";
				} else {
					...
					fwrite($archivo, "MESA: " . $singleorderinfo->tablename . "\r\n");
					fwrite($archivo, "CLIENTE: " . $singleorderinfo->customer_name . "\r\n");
					fwrite($archivo, "ATENDIO: " . $singleorderinfo->firstname . " " . $singleorderinfo->lastname . "\r\n");
					fwrite($archivo, "AREA: " . $kitchen->kitchen_name . "\r\n");
					fwrite($archivo, "APERTURA: " . $singleorderinfo->order_date . " " . $singleorderinfo->order_time . "\r\n");
					fwrite($archivo, "COMENTARIO: " . $singleorderinfo->comentario . "\r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "CANT.x PRODUCTO                           PRECIO\r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					$subtotal = 0;
					foreach ($iteminfo as $item) {
						$total = 48 - (strlen($item->menuqty . "x " . substr($item->variantName, 0, 24)) + strlen(($item->price * $item->menuqty) . ""));
						$spaces = str_repeat(" ", $total);
						//if ($item->kitchenid == $kitchen->kitchenid) {
						fwrite($archivo, $item->menuqty . "x " . substr($item->variantName, 0, 24) . $spaces . ($item->price * $item->menuqty) . "\r\n");
						//$printer-> text("----- ".$item->variantName."\n");
						if ($item->add_on_name != '') {
							fwrite($archivo, "   (A) " . $item->addonsqty . "x " . $item->add_on_name . "\r\n");
						}
					...
					fwrite($archivo, "------------------------------------------------\r\n");
					$total = 48 - (strlen('SUB-TOTAL') + strlen(($subtotal) . ""));
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "SUB-TOTAL" .  $spaces . $subtotal . " \r\n");
					$total = 48 - (strlen('IMPUESTO') + strlen(round($billinfo->VAT, 2) . ""));
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "IMPUESTO" . $spaces . round($billinfo->VAT, 2) . " \r\n");
					...
					fwrite($archivo, "------------------------------------------------\r\n");
					$total = 48 - (strlen('TOTAL') + strlen(($singleorderinfo->totalamount) . ""));
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "TOTAL" . $spaces . $singleorderinfo->totalamount . " \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					$total = 48 - (strlen('TASA') + strlen(($currency_tasa[0]["curr_rate"]) . ""));
					$spaces = str_repeat(" ", $total);
					...
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "TOTAL " . $currency_tasa[0]["curr_icon"] . $spaces . round($singleorderinfo->totalamount * $currency_tasa[0]["curr_rate"], 2) . " \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "FECHA: " . $DateAndTime . "\r\n");
					fwrite($archivo, "IMPRESORA: " . $kitchen->ip . "\r\n");
					fwrite($archivo, "EQUIPO: " . $this->getRealIP() . "\r\n");
		...
		//*********************************************************** */
		if ($infoparams[3]["active"] ==  1) { //Precuenta por maquina fiscal activa
			try {
				$path = $infoparams[2]["value"] . "BemaFI32.cmd";
				$archivo = fopen($path, "wb");
				if ($archivo == false) {
					echo "Error al crear el archivo";
				} else {
					// Escribir en el archivo:
					fwrite($archivo, "009|%\r\n");
					fwrite($archivo, "072|V-20.534.647 - " . $singleorderinfo->customer_name . " - MESERO: " . $singleorderinfo->first_name . " - ORDEN: " . $singleorderinfo->order_id . " - MESA" . $singleorderinfo->table_no . "|\r\n");
					//-----------------------------------------
					foreach ($iteminfo as $item) {
						fwrite($archivo, "072|" . $item->row_id . " - " . $item->variantName . " - " . $item->menuqty . " - " . $item->price . "|\r\n");
						//fwrite($archivo, "089|".."|".."|".$tax."|".$comando."|".( * 1000)."|2|".($item->price * 100)."|$|0.00|*r\n");					
						/*fwrite($archivo, $item->menuqty . "x " . $item->variantName . "\r\n");
							//$printer-> text("----- ".$item->variantName."\n");
							if ($item->add_on_name != '') {
								fwrite($archivo, "   (A) " . $item->addonsqty . "x " . $item->add_on_name . "\r\n");
					...
					//======================================================================
					//fwrite($archivo, "040|D|%|000|r\n");
					//======================================================================

					//fwrite($archivo, "023|EFECTIVO|150|r\n");
					//======================================================================================
					//fwrite($archivo, "082|Gracias por su compra|r\n");
					fwrite($archivo, "031| \n");
					/*foreach ($alliteminfo as $item) {
						if ($item->kitchenid == $kitchen->kitchenid) {
							fwrite($archivo, $item->menuqty . "x " . $item->variantName . "\r\n");
	...
	public function checkorder()
	{
		...
		$data['title'] = "Kitchen Dashboard";
		$data['kitchenid'] = $kid;
		$data['orderinfo'] = $this->order_model->kitchen_ajaxorderinfoall($orderid);
		//$data['orderinfo'] = $this->order_model->read('*', 'customer_order', array('order_id' => $orderid));
	...
	public function paymultiple($type)
	{
		...
		$updatetprebill = array(
			...
			'change' => $paidamount - $this->input->post('grandtotal', true)
		);
		...
		$data['page']   = "updateorderlist";
		$infoparams = $this->db->select('*')->from('system_param')->get()->result_array();
		$infotasa = $this->db->select('*')->from('currency')->where('is_secondary_currency', 1)->get()->result_array();
		$singleorderinfo = $this->order_model->kitchen_ajaxorderinfoall($orderid);
		//================================================
		if ($infoparams[4]['value'] == '0') { //si usa maquina fiscal
			if (round($discount, 2) < round($singleorderinfo->totalamount, 2)) {
				//Imprimimos Factura
				//================================================
				$alliteminfo = $this->order_model->customerorder($orderid);
				...
				try {
					...
					if ($archivo == false) {
						...
						// Escribir en el archivo:
						fwrite($archivo, "009|%\r\n");
						fwrite($archivo, "272|" . $singleorderinfo->cuntomer_no . "|" . $singleorderinfo->customer_name . "| \r\n");
						//-----------------------------------------
						foreach ($alliteminfo as $item) {
							if ($item->productvat != 0) {
								$tax = (($item->productvat) * 100);
								$comando = "F";
							} else {
								$tax = "II";
								$comando = "F";
							...
							$variant_name = "";
							if (strlen($item->variantName) > 20) {
								$variant_name = substr($item->variantName, 0, 20);
							} else {
								$variant_name = $item->variantName;
							}
							fwrite($archivo, "089|" . $item->row_id . "|" . $variant_name . "|" . $tax . "|" . $comando . "|" . ($item->menuqty * 1000) . "|2|" . (round($item->price * $infotasa[0]["curr_rate"], 2) * 100) . "|$|0.00|*r\n");
							/*fwrite($archivo, $item->menuqty . "x " . $item->variantName . "\r\n");
						//$printer-> text("----- ".$item->variantName."\n");*/
							if ($item->add_on_name != '') {
								fwrite($archivo, "089|" . $item->row_id . "|(A)" . $item->add_on_name . "|" . $tax . "|" . $comando . "|" . ($item->addonsqty * 1000) . "|2|" . (round($item->price_add * $infotasa[0]["curr_rate"], 2) * 100) . "|$|0.00|*r\n");
								//fwrite($archivo, "   (A) " . $item->addonsqty . "x " . $item->add_on_name . "\r\n");
							}
							/*
						if ($item->notes != '') {
						...
						if ($billinfo->service_charge != 0) {
							fwrite($archivo, "089|000|SERVICIO|II|F|1000|2|" . (round($billinfo->service_charge * $infotasa[0]["curr_rate"], 2) * 100) . "|$|0.00|*r\n");
						}
						//======================================================================
						fwrite($archivo, "040|D|%|000|r\n");
						//======================================================================
						//MEDIO DE PAGO
						$this->db->select('payment_method.*,multipay_bill.*');
						...
						foreach ($result as $item) {
							fwrite($archivo, "023|" . $item->payment_method . "|" . (round($item->amount * $infotasa[0]["curr_rate"], 2) * 100) . "|r\n");
						}
						//fwrite($archivo, "023|EFECTIVO|150|r\n");
						//======================================================================================
						fwrite($archivo, "082|Gracias por su compra - TASA: " . $infotasa[0]["curr_rate"] . " - MESERO: " . $singleorderinfo->firstname . " - ORDEN: " . $singleorderinfo->order_id . " - MESA: " . $singleorderinfo->tablename . " |r\n");
						//fwrite($archivo, "082|TASA DEL DIA: ".$infoparams[0]["value"]."Bs|r\n");
						fwrite($archivo, "004|r\n");
						fwrite($archivo, "274|%\n");
						/*foreach ($alliteminfo as $item) {
					if ($item->kitchenid == $kitchen->kitchenid) {
						fwrite($archivo, $item->menuqty . "x " . $item->variantName . "\r\n");
				...
				foreach ($kitchenlist as $kitchen) {
					...
					try {
						...
						$path = $infoparams[1]["value"] . "P_" . $kitchen->ip . "-" . $DateAndTimeId . "-GERENCIAL.printer";
						$archivo = fopen($path, "wb");
						if ($archivo == false) {
							echo "Error al crear el archivo";
						} else {
							// Escribir en el archivo:
							fwrite($archivo, "------------------------------------------------\r\n");
							fwrite($archivo, "CONSUMO GERENCIAL: \r\n");
							fwrite($archivo, "------------------------------------------------\r\n");
							fwrite($archivo, "ORDEN: " . $singleorderinfo->order_id . "\r\n");
							fwrite($archivo, "MESA: " . $table . "\r\n");
							fwrite($archivo, "CLIENTE: " . $singleorderinfo->customer_name . "\r\n");
							...
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "IMPUESTO" . $spaces . round($billinfo->VAT, 2) . " \r\n");
							$total = 48 - (strlen('SERVICIO') + strlen(round($billinfo->service_charge, 2) . ""));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "SERVICIO" . $spaces . round($billinfo->service_charge, 2) . " \r\n");
							fwrite($archivo, "------------------------------------------------\r\n");
							$total = 48 - (strlen('TOTAL') + strlen(($singleorderinfo->totalamount) . ""));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "TOTAL" . $spaces . $singleorderinfo->totalamount . " \r\n");
							...
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "TOTAL " . $currency_tasa[0]["curr_icon"] . $spaces . round($singleorderinfo->totalamount * $currency_tasa[0]["curr_rate"], 2) . " \r\n");
							$total = 48 - (strlen('DESCUENTO') + strlen((round($billinfo->discount, 2)) . ""));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "DESCUENTO" . $spaces .  round($billinfo->discount, 2) . " \r\n");
							fwrite($archivo, "------------------------------------------------\r\n");
							fwrite($archivo, "FECHA: " . $DateAndTime . "\r\n");
							fwrite($archivo, "IMPRESORA: " . $kitchen->ip . "\r\n");
							fwrite($archivo, "EQUIPO: " . $this->getRealIP() . "\r\n");
			...
			foreach ($kitchenlist as $kitchen) {
				...
				try {
					...
					$path = $infoparams[1]["value"] . "P_" . $kitchen->ip . "-" . $DateAndTimeId . "-FACTURA.printer";
					$archivo = fopen($path, "wb");
					if ($archivo == false) {
						echo "Error al crear el archivo";
					} else {
						// Escribir en el archivo:
						fwrite($archivo, "------------------------------------------------\r\n");
						fwrite($archivo, "TICKET DE FACTURA: \r\n");
						fwrite($archivo, "------------------------------------------------\r\n");
						fwrite($archivo, "ORDEN: " . $singleorderinfo->order_id . "\r\n");
						fwrite($archivo, "MESA: " . $table . "\r\n");
						fwrite($archivo, "CLIENTE: " . $singleorderinfo->customer_name . "\r\n");
		...
		$this->db->where('order_id', $orderid);
		$this->db->update('customer_order', $updateJournal);
		$this->db->query("update station set is_update=NULL");
		//================================================
		$view = $this->posprintdirect($orderid);
		echo $view;
	...
	public function changeMargeorder()
	{
		...
		//$order_id_ = $mydata['billinfo'][0]->order_id;
		//==================================================================================
		$infoparams = $this->db->select('*')->from('system_param')->get()->result_array();
		$infotasa = $this->db->select('*')->from('currency')->where('is_secondary_currency', 1)->get()->result_array();
		//==================================================================================
		foreach ($this->input->post('order', true) as $order_id) {
			$singleorderinfo = $this->order_model->kitchen_ajaxorderinfoall($order_id);
			if (round($discount, 2) < round($singleorderinfo->totalamount, 2)) {
				//Imprimimos Factura
				//================================================
				$alliteminfo = $this->order_model->customerorder($order_id);
				//================================================
				try {
					$path = $infoparams[2]["value"] . "BemaFI32.cmd";
					$archivo = fopen($path, "wb");
				...
				$iteminfo = $this->order_model->customerorder($order_id);
				//$alliteminfo = $this->order_model->customerorder($orderid);
				//$infoparams = $this->db->select('*')->from('system_param')->get()->result_array();
				$currency_tasa = $this->db->select('*')->from('currency')->where('is_secondary_currency', 1)->get()->result_array();
				//Imprimir comanda de cocina
				...
				foreach ($kitchenlist as $kitchen) {
					...
					try {
						...
						if ($archivo == false) {
							...
							$subtotal = 0;
							foreach ($iteminfo as $item) {
								$total = 48 - (strlen($item->menuqty . "x " . $item->variantName) + strlen(($item->price * $item->menuqty) . ""));
								$spaces = str_repeat(" ", $total);
								//return $spaces;
								//if ($item->kitchenid == $kitchen->kitchenid) {
								fwrite($archivo, $item->menuqty . "x " . $item->variantName . $spaces . ($item->price * $item->menuqty) . "\r\n");
								//$printer-> text("----- ".$item->variantName."\n");
								if ($item->add_on_name != '') {
									fwrite($archivo, "   (A) " . $item->addonsqty . "x " . $item->add_on_name . "\r\n");
								}
							...
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "SUB-TOTAL" . $spaces . $subtotal . " \r\n");
							$total = 48 - (strlen('IMPUESTO') + strlen(($billinfo->VAT) . ""));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "IMPUESTO" . $spaces . ($billinfo->VAT) . " \r\n");
							$total = 48 - (strlen('SERVICIO') + strlen(($billinfo->service_charge) . ""));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "SERVICIO" . $spaces . ($billinfo->service_charge) . " \r\n");
							fwrite($archivo, "------------------------------------------------\r\n");
							$total = 48 - (strlen('TOTAL') + strlen(($singleorderinfo->totalamount) . ""));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "TOTAL" . $spaces . $singleorderinfo->totalamount . " \r\n");
							...
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "TOTAL " . $currency_tasa[0]["curr_icon"] . $spaces . round($singleorderinfo->totalamount * $currency_tasa[0]["curr_rate"], 2) . " \r\n");
							$total = 48 - (strlen('DESCUENTO') + strlen(($billinfo->discount) . ""));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, "DESCUENTO" . $spaces .  $billinfo->discount . " \r\n");
							fwrite($archivo, "------------------------------------------------\r\n");
							fwrite($archivo, "FECHA: " . $DateAndTime . "\r\n");
							fwrite($archivo, "IMPRESORA: " . $kitchen->ip . "\r\n");
							fwrite($archivo, "EQUIPO: " . $this->getRealIP() . "\r\n");
	...
	public function delete_table_details($id)
	{
		...
		$this->db->where('order_id', $id)->delete('table_details');
		echo '1';
	}
	...
	public function posprintdirectsub($id)
	{
		...
		//Imprimimos Factura
		//================================================
		$alliteminfo = $this->order_model->customerorder($id);
		$singleorderinfo = $this->order_model->kitchen_ajaxorderinfoall($id);
		//================================================
		try {
			$path = $infoparams[2]["value"] . "BemaFI32.cmd";
			...
			if ($archivo == false) {
				...
				//fwrite($archivo, "023|EFECTIVO|150|r\n");
				//======================================================================================
				fwrite($archivo, "082|Gracias por su compra - TASA: " . $infotasa[0]["curr_rate"] . " - MESERO: " . $singleorderinfo->firstname . " - ORDEN: " . $singleorderinfo->order_id . " - MESA" . $singleorderinfo->table_no . " |r\n");
				//fwrite($archivo, "082|TASA DEL DIA: ".$infoparams[0]["value"]."Bs|r\n");
				fwrite($archivo, "004|r\n");
				fwrite($archivo, "274|%\n");
				/*foreach ($alliteminfo as $item) {
	...
	public function closecashregisterz()
	{
		//Imprimimos Z y cerramos la jornada
		//================================================
		//$alliteminfo = $this->order_model->customerorderkitchenAll($orderid);
		//$singleorderinfo = $this->order_model->kitchen_ajaxorderinfoall($orderid);
		//================================================
		...
		try {
			...
			if ($archivo == false) {
				...
				// Escribir en el archivo:
				/*fwrite($archivo, "009|%\r\n");
				fwrite($archivo, "072|IMPRIMIR REPORTE Z|\r\n");
				fwrite($archivo, "031|\r\n");*/
				//================================================
				fwrite($archivo, "009|%\r\n"); //071| 
				fwrite($archivo, "071|\r\n");
			...
			$this->db->update('journal', $journal_update);
			//===========================================================
			// Cerrar el archivo:
			fclose($archivo);
		}
	...
	public function closecashregisterz_prev()
	{
		//Imprimimos Z
		//================================================
		//$alliteminfo = $this->order_model->customerorderkitchenAll($orderid);
	...
	public function printReportManagement()
	{
		//Imprimir comanda de cocina
		//****************************************************/
		$infoparams = $this->db->select('*')->from('system_param')->get()->result_array();
		...
		foreach ($kitchenlist as $kitchen) {
			...
			try {
				...
				if ($archivo == false) {
					...
					// Escribir en el archivo:
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "INFORME GERENCIAL: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "RESUMEN DE ORDENES: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");

					$total_order = 0;
					$total_order = $this->db->query('select sum(totalamount) as total from customer_order where journal_id=' . $_SESSION['journal'] . " and order_status = 4")->row();

					$product_vallet = $this->db->query("select id_vallet_parking_product from setting where id = 1")->row();

					$total_order_vallet_parking = 0;
					$total_order_vallet_parking = $this->db->query("select sum(price) as total from order_menu om where menu_id = " . $product_vallet->id_vallet_parking_product . " and order_id in(select order_id as total from customer_order where journal_id=" . $_SESSION['journal'] . " and order_status = 4)")->row();

					$total_order_deleted = 0;
					$total_order_deleted = $this->db->query('select sum(totalamount) as total from customer_order where journal_id=' . $_SESSION['journal'] . " and order_status = 5")->row();

					$total_order_open = 0;
					$total_order_open = $this->db->query('select sum(totalamount) as total from customer_order where journal_id=' . $_SESSION['journal'] . " and order_status = 1")->row();

					$total_order_management = 0;
					$total_order_management = $this->db->query("select sum(discount) as total from bill where order_id in(select order_id from customer_order where journal_id=" . $_SESSION['journal'] . " and order_status = 4 and totalamount = 0)")->row();

					$total_order_service = 0;
					$total_order_service = $this->db->query("select sum(service_charge) as total from bill where order_id in(select order_id from customer_order where journal_id=" . $_SESSION['journal'] . " and order_status = 4)")->row();

					$payment_method =  $this->db->query("select pm.payment_method, count(multipay_id), sum(amount) as total from multipay_bill left join payment_method pm on pm.payment_method_id = multipay_bill.payment_type_id where order_id in(select order_id as total from customer_order where journal_id=" . $_SESSION['journal'] . " and order_status = 4) group by payment_type_id")->result();


					$total = 48 - (strlen('TOTAL CERRADAS') + strlen((round($total_order->total, 2))));
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "TOTAL CERRADAS" . $spaces . round($total_order->total, 2) . "\r\n");

					$total = 48 - (strlen('TOTAL ELIMINADAS') + strlen((round($total_order_deleted->total, 2))));
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "TOTAL ELIMINADAS" . $spaces . round($total_order_deleted->total, 2) . "\r\n");

					$total = 48 - (strlen('TOTAL ABIERTAS') + strlen((round($total_order_open->total, 2))));
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "TOTAL ABIERTAS" . $spaces . round($total_order_open->total, 2) . "\r\n");

					$total = 48 - (strlen('TOTAL CONSUMOS G.') + strlen((round($total_order_management->total, 2))));
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "TOTAL CONSUMOS G." . $spaces . round($total_order_management->total, 2) . "\r\n");

					$total = 48 - (strlen('TOTAL SERVICIO') + strlen((round($total_order_service->total, 2))));
					$spaces = str_repeat(" ", $total);
					fwrite($archivo, "TOTAL SERVICIO" . $spaces . round($total_order_service->total, 2) . "\r\n");

					if (!empty($total_order_vallet_parking)) {
						$total = 48 - (strlen('TOTAL ESTACIONAMIENTO') + strlen((round($total_order_vallet_parking->total, 2))));
						$spaces = str_repeat(" ", $total);
						fwrite($archivo, "TOTAL ESTACIONAMIENTO" . $spaces . round($total_order_vallet_parking->total, 2) . "\r\n");
					}

					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "RESUMEN DE MEDIOS DE PAGO: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");

					foreach ($payment_method as $value) {
						$total = 48 - (strlen($value->payment_method) + strlen((round($value->total, 2))));
						$spaces = str_repeat(" ", $total);
						fwrite($archivo, $value->payment_method . $spaces . round($value->total, 2) . "\r\n");
					}

					$sales_category = $this->db->query("select ic.Name, sum(om.price) as total from order_menu as om left join item_foods as if2 on om.menu_id = if2.ProductsID left join item_category as ic on if2.CategoryID = ic.CategoryID where order_id in(select order_id as total from customer_order where journal_id=" . $_SESSION['journal'] . " and order_status = 4) group by ic.CategoryID")->result();
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "VENTA POR CATEGORIA\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					if (!empty($sales_category)) {
						foreach ($sales_category as $value) {
							$total = 48 - (strlen($value->Name) + strlen((round($value->total, 2))));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, $value->Name . $spaces . round($value->total, 2) . "\r\n");
						}
					}

					$cash_expenses =  $this->db->query("select * from cash_expenses where journal_id = " . $_SESSION['journal'])->result();
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "RESUMEN DE RETIROS: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					if (!empty($cash_expenses)) {
						foreach ($cash_expenses as $value) {
							$total = 48 - (strlen($value->date) + strlen((round($value->amount, 2))));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, $value->date . $spaces . round($value->amount, 2) . "\r\n");
						}
					}

					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "ORDENES ABIERTAS: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					$total_order_open_list = $this->db->query("select co.*, ci.* from customer_order as co left join customer_info as ci on co.customer_id = ci.customer_id where co.journal_id=" . $_SESSION['journal'] . " and co.order_status = 1")->result();
					if (!empty($total_order_open_list)) {
						foreach ($total_order_open_list as $value) {
							$total = 48 - (strlen($value->order_id . " ") + strlen($value->customer_name) + strlen((round($value->totalamount, 2))));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, $value->order_id . " " . $value->customer_name . $spaces . round($value->totalamount, 2) . "\r\n");
						}
					}
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "ORDENES ELIMINADAS: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					$total_order_deleted_list = $this->db->query('select * from customer_order where journal_id=' . $_SESSION['journal'] . " and order_status = 5")->result();
					if (!empty($total_order_deleted_list)) {
						foreach ($total_order_deleted_list as $value) {
							$total = 48 - (strlen($value->order_id) + strlen((round($value->totalamount, 2))));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, $value->order_id . $spaces . round($value->totalamount, 2) . "\r\n");
						}
					}
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "ORDENES CONSUMO GERENCIAL: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					$total_order_management_list = $this->db->query("select * from bill where order_id in(select order_id from customer_order where journal_id=" . $_SESSION['journal'] . " and order_status = 4 and bill_amount = 0)")->result();
					if (!empty($total_order_management_list)) {
						foreach ($total_order_management_list as $value) {
							$total = 48 - (strlen($value->order_id) + strlen((round($value->bill_amount, 2))));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, $value->order_id . $spaces . round($value->bill_amount, 2) . "\r\n");
						}
					}

					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "MESAS TRANSFERIDAS: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					$transfer_table_list = $this->db->query("select order_origin, date, (select tablename from rest_table rt where tableid = th.table_origin) nameOrigin,(select tablename from rest_table rt where tableid = th.table_destiny) nameDestiny from transfer_history th where th.journal_id =" . $_SESSION['journal'] . " and th.type = 1")->result();
					if (!empty($transfer_table_list)) {
						foreach ($transfer_table_list as $value) {
							$total = 48 -  (strlen($value->date) + strlen(" " . $value->order_origin) + strlen((($value->nameOrigin))) + strlen(" a ") + strlen((($value->nameDestiny))));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, $value->date . " " . $value->order_origin . $spaces . ($value->nameOrigin) . " a " . ($value->nameDestiny) . "\r\n");
						}
					}

					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "PRODUCTOS TRANSFERIDOS: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					$transfer_table_list = $this->db->query("select order_origin, date, order_destiny from transfer_history th where th.journal_id =" . $_SESSION['journal'] . " and th.type = 2")->result();
					if (!empty($transfer_table_list)) {
						foreach ($transfer_table_list as $value) {
							$total = 48 -  (strlen($value->date) + strlen((($value->order_origin))) + strlen(" a ") + strlen((($value->order_destiny))));
							$spaces = str_repeat(" ", $total);
							fwrite($archivo, $value->date . $spaces . ($value->order_origin) . " a " . ($value->order_destiny) . "\r\n");
						}
					}



					$tips =  $this->db->query("select * from tips where journal_id = " . $_SESSION['journal'])->result();
					fwrite($archivo, "------------------------------------------------\r\n");
					fwrite($archivo, "RESUMEN DE PROPINAS: \r\n");
					fwrite($archivo, "------------------------------------------------\r\n");
					if (!empty($tips)) {
					...
					//fwrite($archivo, "------------------------------------------------\r\n");
					//fwrite($archivo, "CANT.x PRODUCTO                                 \r\n");
					/*$subtotal = 0;
					//=======================================================================================
					fwrite($archivo, "------------------------------------------------\r\n");
					$total = 48 - (strlen('SUB-TOTAL') + strlen(($subtotal) . ""));
	...
	//update table transfer
	public function table_transfer($order, $tablefinal, $tableOrigin)
	{
		//===========================================================
		$table_update = array(
			'table_no'           => $tablefinal
		...
		$this->db->update('customer_order', $table_update);
		//===========================================================
		$table_update = array(
			'table_id'           => $tablefinal
		);
		$this->db->where('order_id', $order);
		$this->db->update('table_details', $table_update);
		//===========================================================
		$dataLog = array(
			'order_origin'           => $order,
			'table_destiny'          => $tablefinal,
			'table_origin'           => $tableOrigin,
			'journal_id'             => $_SESSION['journal'],
			'type'                   => 1
		);
		$this->db->insert('transfer_history', $dataLog);
		$this->db->query("update station set is_update=NULL");
		//===========================================================
	}

	//update table transfer
	public function products_transfer($order, $order_final)
	{
		//===========================================================
		$table_update = array(
		...
		$this->db->where('order_id', $order);
		$this->db->update('order_menu', $table_update);
		//===========================================================
		//===========================================================
		$dataLog = array(
			'order_origin'           => $order,
			'order_destiny'          => $order_final,
			'journal_id'             => $_SESSION['journal'],
			'type'                   => 2
		);
		$this->db->insert('transfer_history', $dataLog);
	...
	{
		//===========================================================
		$this->db->query("update customer_order set user_view = NULL");
		//===========================================================
	}

	//marcar orden
	public function markOrder($order_id, $value)
	{
		//===========================================================
		$this->db->query("update customer_order set is_mark = " . $value . " where order_id = " . $order_id);
		//===========================================================
	}

	//update orden product qty
	public function updateProductQty($row_id, $value, $orderid)
	{
		//===========================================================
		$this->db->query("update order_menu set menuqty = " . $value . " where row_id = " . $row_id);
		//===========================================================
		$iteminfo = $this->order_model->customerorder($orderid);
		$data['paymentmethod']   = $this->order_model->pmethod_dropdown();
		$data['billinfo']	   = $this->order_model->billinfo($orderid);
	...
	public function validatePasswordMain($password)
	{
		//$password = $this->input->post('password');
		$validate_pass = "";
		$dato = $this->db->select('*')->from('user')->where('password', md5($password))->where('is_admin', 1)->get()->row();
		if (empty($dato)) {
			$dato = $this->db->select('*')->from('setting')->where('id', 1)->get()->row();
			$validate_pass = $dato->password_main;
		} else {
	...
	{
		$dato = $this->db->select('*')->from('system_param')->where('id', $param)->get()->row();
		echo $dato->value;
	}

	public function filterOrdersType($valor)
	{
		$_SESSION["view_ongoingorder"] = $valor;
		$_SESSION["search_orders"] = 0;
	}

	public function filterOrdersSearch($valor)
	{
		$_SESSION["search_orders"] = $valor;
		$_SESSION["view_ongoingorder"] = 0;
	}

	public function validateUpdateOrder()
	{
		//$dato = $this->db->select('count(order_id) as value')->from('customer_order')->where('is_update', 1)->get()->row();
		$dato = $this->db->query("select is_update as value from station where ip = '" . $this->getRealIP() . "'")->row();
		if (!empty($dato)) {
			$this->db->query("update station set is_update=1 where ip = '" . $this->getRealIP() . "'");
			echo $dato->value;
		} else {
			echo '1';
		}
	...
	public function validateProductNoSend($order_id)
	{
		$dato = $this->order_model->read('*', 'order_menu', array('order_id' => $order_id, 'isupdate' => 1));
		echo $dato;
	}

	public function retirarCash($amount)
	{
		$data = array(
			'amount'           => $amount,
			'journal_id'       => $_SESSION['journal']
		);
		$this->db->insert('cash_expenses', $data);
	}

	public function addTips($amount)
	{
		$data = array(
			'amount'           => $amount,
			'journal_id'       => $_SESSION['journal']
