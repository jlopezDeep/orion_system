...
class Order_model extends CI_Model
{
	...
	{
		$this->db->select('item_foods.*, item_category.color_category_grid');
		$this->db->from('item_foods');
		$this->db->join('item_category', 'item_category.CategoryID = item_foods.CategoryID', 'left');
	...
	public function searchdropdown($pname = null)
	{
		...
		$this->db->from('item_foods');
		$this->db->join('variant', 'item_foods.ProductsID=variant.menuid', 'left');
		$where = "(item_foods.ProductName like '%" . $pname . "%' OR item_foods.descrip like '%" . $pname . "%') and item_foods.ProductsIsActive=1";
		//$this->db->like('item_foods.ProductName',$pname);
		//$this->db->like('item_foods.descrip',$pname);
		$this->db->where($where);
		$query = $this->db->get();
		$itemlist = $query->result();
	...
	{
		$this->db->select('customer_order.*,customer_info.customer_name,customer_type.customer_type,user.firstname,user.lastname,rest_table.tablename');
		$this->db->from('customer_order');
		...
		$this->db->join('user', 'customer_order.waiter_id=user.id', 'left');
	...
	public function customerorder($id, $nststus = null)
	{
		...
		$sql = "SELECT order_menu.istransfer, item_foods.productvat, add_ons.price as price_add, add_ons.*, add_ons.add_on_name, tbl_kitchen.*, order_menu.row_id,order_menu.order_id,order_menu.groupmid as menu_id,order_menu.notes,order_menu.add_on_id,order_menu.addonsqty,order_menu.groupvarient as varientid,order_menu.addonsuid,order_menu.qroupqty as menuqty,order_menu.price as price,order_menu.isgroup,order_menu.food_status,order_menu.allfoodready,order_menu.isupdate, item_foods.ProductName, variant.variantid, variant.variantName, variant.price as mprice FROM order_menu LEFT JOIN item_foods ON order_menu.groupmid=item_foods.ProductsID LEFT JOIN variant ON order_menu.groupvarient=variant.variantid LEFT JOIN tbl_kitchen ON item_foods.kitchenid = tbl_kitchen.kitchenid LEFT JOIN add_ons ON order_menu.add_on_id = add_ons.add_on_id  WHERE {$where} AND order_menu.isgroup=1 Group BY order_menu.groupmid UNION SELECT order_menu.istransfer, item_foods.productvat, add_ons.price as price_add, add_ons.*, add_ons.add_on_name, tbl_kitchen.*,order_menu.row_id,order_menu.order_id,order_menu.menu_id as menu_id,order_menu.notes,order_menu.add_on_id,order_menu.addonsqty,order_menu.varientid as varientid,order_menu.addonsuid,order_menu.menuqty as menuqty,order_menu.price as price,order_menu.isgroup,order_menu.food_status,order_menu.allfoodready,order_menu.isupdate, item_foods.ProductName, variant.variantid, variant.variantName, variant.price as mprice FROM order_menu LEFT JOIN item_foods ON order_menu.menu_id=item_foods.ProductsID LEFT JOIN tbl_kitchen ON item_foods.kitchenid =  tbl_kitchen.kitchenid LEFT JOIN variant ON order_menu.varientid=variant.variantid LEFT JOIN add_ons ON order_menu.add_on_id = add_ons.add_on_id  WHERE {$where} AND order_menu.isgroup=0";
		$query = $this->db->query($sql);
	...
	{
		$this->db->select('add_ons.*, order_menu.*,item_foods.ProductName,item_foods.kitchenid,item_foods.cookedtime,variant.variantid,variant.variantName,variant.price');
		$this->db->from('order_menu');
		$this->db->join('item_foods', 'order_menu.menu_id=item_foods.ProductsID', 'left');
		$this->db->join('variant', 'order_menu.varientid=variant.variantid', 'left');
		$this->db->join('add_ons', 'order_menu.add_on_id=add_ons.add_on_id', 'left');
		$this->db->where('order_menu.order_id', $id);
	...
	public function customerorderkitchenAll($id)
	{
		$this->db->select('item_foods.productvat, tbl_kitchen.*, add_ons.*, order_menu.*,item_foods.ProductName,item_foods.kitchenid,item_foods.cookedtime,variant.variantid,variant.variantName,variant.price');
		$this->db->from('order_menu');
		$this->db->join('item_foods', 'order_menu.menu_id=item_foods.ProductsID', 'left');
		$this->db->join('variant', 'order_menu.varientid=variant.variantid', 'left');
		$this->db->join('add_ons', 'order_menu.add_on_id=add_ons.add_on_id', 'left');
		$this->db->join('tbl_kitchen', 'item_foods.kitchenid=tbl_kitchen.kitchenid', 'left');
		$this->db->where('order_menu.order_id', $id);
		//$this->db->group_by('item_foods.kitchenid');
		//$this->db->group_by('order_menu.add_on_id');
		//$this->db->group_by('order_menu.notes');
		//$this->db->where('item_foods.kitchenid',$kitchen);
		$query = $this->db->get();
		$orderinfo = $query->result();
		return $orderinfo;
	...
	{
		$this->db->select('bill.service_charge, customer_order.*,customer_info.customer_name, customer_info.cuntomer_no, customer_type.customer_type,user.firstname,user.lastname,rest_table.tablename');
		$this->db->from('customer_order');
		$this->db->join('customer_info', 'customer_order.customer_id=customer_info.customer_id', 'left');
		$this->db->join('customer_type', 'customer_order.cutomertype=customer_type.customer_type_id', 'left');
		$this->db->join('user', 'customer_order.waiter_id=user.id', 'left');
	...
	//JOSE LISTA DE ORDENES EN MARCHA
	public function get_ongoingorder()
	{
		$cdate = date("Y-m-d", strtotime("- 1 day"));
		$today = date("Y-m-d");
		$where = "";
		//customer_order.order_date Between '" . $cdate . "' AND '" . $today . "' AND 
		if ($_SESSION['view_ongoingorder'] == 0) {
			$where = "((customer_order.order_status = 1 OR customer_order.order_status = 2 OR customer_order.order_status = 3) AND ((customer_order.cutomertype = 99 AND customer_order.orderacceptreject = 1) || (customer_order.cutomertype = 3 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 4 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 1 || customer_order.orderacceptreject != 1)))";
		}
		if ($_SESSION['view_ongoingorder'] == 2) {
			$where = "customer_order.table_no = 0 AND ((customer_order.order_status = 1 OR customer_order.order_status = 2 OR customer_order.order_status = 3) AND ((customer_order.cutomertype = 99 AND customer_order.orderacceptreject = 1) || (customer_order.cutomertype = 3 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 4 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 1 || customer_order.orderacceptreject != 1)))";
		}
		...
		if ($_SESSION['search_orders'] != 0) {
			$where = "(customer_order.order_id like '%" . $_SESSION['search_orders'] . "%' or rest_table.tablename like '%" . $_SESSION['search_orders'] . "%' or customer_order.comentario like '%" . $_SESSION['search_orders'] . "%') AND ((customer_order.order_status = 1 OR customer_order.order_status = 2 OR customer_order.order_status = 3) AND ((customer_order.cutomertype = 99 AND customer_order.orderacceptreject = 1) || (customer_order.cutomertype = 3 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 4 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 1 || customer_order.orderacceptreject != 1)))";
		}

		$sql = "SELECT customer_order.*, (select count(order_id) from order_menu where order_id = customer_order.order_id and isupdate = 1) as nosendproducts, customer_order.order_id as mid,customer_info.customer_name,customer_type.customer_type,user.firstname,user.lastname,rest_table.tablename FROM customer_order Left JOIN customer_info ON customer_order.customer_id=customer_info.customer_id Left Join customer_type ON customer_order.cutomertype=customer_type.customer_type_id left join user ON customer_order.waiter_id=user.id Left Join rest_table ON customer_order.table_no=rest_table.tableid Where {$where} AND customer_order.marge_order_id IS NULL UNION SELECT customer_order.*, (select count(order_id) from order_menu where order_id = customer_order.order_id and isupdate = 1) as nosendproducts, customer_order.order_id as mid,customer_info.customer_name,customer_type.customer_type,user.firstname,user.lastname,rest_table.tablename FROM customer_order Left JOIN customer_info ON customer_order.customer_id=customer_info.customer_id Left Join customer_type ON customer_order.cutomertype=customer_type.customer_type_id left join user ON customer_order.waiter_id=user.id Left Join rest_table ON customer_order.table_no=rest_table.tableid Where {$where} AND customer_order.marge_order_id IS NOT NULL GROUP BY customer_order.marge_order_id, customer_order.order_id order by mid desc";
	...
	{
		$cdate = date('Y-m-d');
		//customer_order.order_date = '" . $cdate . "'
		$where = "customer_order.table_no = '" . $id . "' AND customer_order.cutomertype !=2 AND (customer_order.order_status = 1 OR customer_order.order_status = 2 OR customer_order.order_status = 3)";
	...
	{
		$column_order = array(null, 'customer_order.saleinvoice', 'customer_info.customer_id', 'customer_type.customer_type', 'CONCAT_WS(" ", user.firstname,user.lastname)', 'rest_table.tablename', 'customer_order.order_date', 'customer_order.totalamount'); //set column field database for datatable orderable
		$column_search = array('customer_order.saleinvoice', 'customer_info.customer_id', 'customer_type.customer_type', 'CONCAT_WS(" ", user.firstname,user.lastname)', 'rest_table.tablename', 'customer_order.order_date', 'customer_order.totalamount'); //set column field database for datatable searchable 
		$order = array('customer_order.order_id' => 'asc');
		$this->db->select('customer_order.*,customer_info.customer_name,customer_info.customer_id,customer_type.customer_type,CONCAT_WS(" ", user.firstname,user.lastname) AS fullname,rest_table.tablename');
		$this->db->from('customer_order');
		$this->db->join('customer_info', 'customer_order.customer_id=customer_info.customer_id', 'left');
		$this->db->join('customer_type', 'customer_order.cutomertype=customer_type.customer_type_id', 'left');
		$this->db->join('user', 'customer_order.waiter_id=user.id', 'left');
	...
	{
		$cdate = date('Y-m-d');
		//customer_order.order_date = '" . $cdate . "' AND 
		$where = "customer_order.cutomertype !=2 AND (customer_order.order_status = 1 OR customer_order.order_status = 2 OR customer_order.order_status = 3)";
	...
	public function kitchen_ongoingorderall()
	{
		$cdate = date('Y-m-d');
		//customer_order.order_date = '" . $cdate . "' AND 
		$where = "((customer_order.order_status = 1 OR customer_order.order_status = 2) AND ((customer_order.cutomertype = 2 AND customer_order.orderacceptreject = 1) || (customer_order.cutomertype = 99 AND customer_order.orderacceptreject = 1) || (customer_order.cutomertype = 3 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 4 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 1 || customer_order.orderacceptreject != 1)))";
		$this->db->select('customer_order.*,customer_info.customer_name,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename');
		$this->db->from('customer_order');
		$this->db->join('customer_info', 'customer_order.customer_id=customer_info.customer_id', 'left');
		$this->db->join('customer_type', 'customer_order.cutomertype=customer_type.customer_type_id', 'left');
	...
	public function counter_ongoingorder()
	{
		$cdate = date('Y-m-d');
		//customer_order.order_date = '" . $cdate . "'
		$where = "(customer_order.order_status = 1 OR customer_order.order_status = 2 OR customer_order.order_status = 3)";
		$this->db->select('customer_order.*,customer_info.customer_name,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename');
		$this->db->from('customer_order');
		$this->db->join('customer_info', 'customer_order.customer_id=customer_info.customer_id', 'left');
		$this->db->join('customer_type', 'customer_order.cutomertype=customer_type.customer_type_id', 'left');
	...
	{
		$column_order = array(null, 'customer_order.saleinvoice', 'customer_info.customer_name', 'customer_type.customer_type', 'user.firstname', 'user.lastname', 'rest_table.tablename', 'customer_order.order_date', 'customer_order.totalamount'); //set column field database for datatable orderable
		$column_search = array('customer_order.saleinvoice', 'customer_info.customer_name', 'customer_type.customer_type', 'user.firstname', 'user.lastname', 'rest_table.tablename', 'customer_order.order_date', 'customer_order.totalamount'); //set column field database for datatable searchable 
		$order = array('customer_order.order_id' => 'asc');

		$cdate = date('Y-m-d');
		$this->db->select('customer_order.*,customer_info.customer_name,customer_type.customer_type,user.firstname,user.lastname,rest_table.tablename,bill.bill_status');
		$this->db->from('customer_order');
		$this->db->join('customer_info', 'customer_order.customer_id=customer_info.customer_id', 'left');
		$this->db->join('customer_type', 'customer_order.cutomertype=customer_type.customer_type_id', 'left');
		$this->db->join('user', 'customer_order.waiter_id=user.id', 'left');
		$this->db->join('rest_table', 'customer_order.table_no=rest_table.tableid', 'left');
		$this->db->join('bill', 'customer_order.order_id=bill.order_id', 'left');
		$this->db->where('customer_order.journal_id', $_SESSION['journal']); //JORNADA AJUSTE 
	...
	{
		$where = "table_id = '" . $id . "' AND delete_at = 0"; //AND created_at= '" . date('Y-m-d') . "'";
		$this->db->select('table_details.*, customer_order.*');
		$this->db->from('table_details');
		$this->db->join('customer_order', 'customer_order.order_id=table_details.order_id', 'left');
		$this->db->where($where);
		$query = $this->db->get();
		$result = $query->result();
		return $result;
	...
	public function checkingredientstock($foodid, $vid, $foodqty)
	{
		...
		if (!empty($checksetitem)) {
			...
			foreach ($groupitemlist as $groupitem) {
				...
				if (empty($productiondetails)) {
					$isavailable = false;
					return 'No se ha definido una receta para este producto' . $groupitem->items;
					break;
				} else {
					foreach ($productiondetails as $productiondetail) {
						...
						if ($stockcheck == 0) {
							return 'Verifque los ingredientes!!. Algunos de los ingredientes que componen este plato no estan disponibles en stock!!!' . $groupitem->items;
						}
						/*end add ingredients*/
					}
				}
		...
		if (!empty($productiondetails)) {
			foreach ($productiondetails as $productiondetail) {
				...
				if ($stockcheck == 0) {
					return 'Verifque los ingredientes!!. Algunos de los ingredientes que componen este plato no estan disponibles en stock!!!';
				}
	...
	//SE AJUSTO EL SUM PARA QUE NO INCLUYERA EL CAMBIO
	public function collectcash($id, $tdate)
	{
		...
		$query = $this->db->get();
		$this->db->select('bill.*,0 payment_type_id,SUM(bill.change * -1) as totalamount,"CAMBIO" payment_method');
		$this->db->from('multipay_bill');
		$this->db->join('bill', 'bill.order_id=multipay_bill.order_id', 'left');
		$this->db->join('payment_method', 'payment_method.payment_method_id=multipay_bill.payment_type_id', 'left');
		$this->db->where('bill.create_by', $id);
		...
		$orderdetails = array_merge($query->result(), $query2->result());
		return $orderdetails;
	}
}
