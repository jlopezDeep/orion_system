...
class Auth extends MX_Controller
{
	...
	public function index()
	{
		...
		$this->form_validation->set_rules('email', display('email'), 'max_length[100]|trim'); //required|valid_email|max_length[100]|trim
		$this->form_validation->set_rules('password', display('password'), 'max_length[32]|md5|trim'); //required|max_length[32]|md5|trim
		$this->form_validation->set_rules('pin', "pin", 'max_length[100]|trim');
		...
		if ($this->form_validation->run()) {
			...
			if (!empty($userData['pin'])) {
				$user = $this->auth_model->checkUserPin($userData);
			} else {
			...
			if ($user->num_rows() > 0) {
				...
				if (!empty($chefid)) {
					...
					redirect('ordermanage/order/pos_invoice'); //'ordermanage/order/counterboard');
				} else {
					redirect('ordermanage/order/pos_invoice'); //'dashboard/home');
				}
			} else {
